...
public class BattingPartnershipService {
	...
	public void PartnershipUpdate(int batsman1Id, int batsman2id, int matchId, int teamId, int runs, int balls,
			boolean status) {

		Optional<BattingPartnership> BattingParnershipExist = partnershipRepository.PartnershipExist(batsman1Id,
				batsman2id, matchId);
		...
		if (BattingParnershipExist.isPresent()) {
			...
			BattingPartnership partnership = new BattingPartnership(matchId, teamId, batsman1Id, batsman2id, runs,
					balls, status);
