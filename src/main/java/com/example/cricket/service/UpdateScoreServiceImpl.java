...
public class UpdateScoreServiceImpl implements UpdateScoreService {
	...
	public ResponseEntity toss(Toss toss) {
		Optional<TeamScore> tossWinner = Optional
				.ofNullable(teamScoreRepository.findByMatchIdAndTeamId(toss.getMatchId(), toss.getTeamId()));
		...
		return ResponseEntity.status(HttpStatus.NOT_FOUND)
				.body(new MainResponse(409, "Match ID or Team ID not found", ""));
		...
		if (currentMatch.isPresent()) {
			...
			if (match.getStatus().equals("Live")) {
				return ResponseEntity.status(HttpStatus.OK)
						.body(new MainResponse(200, "Match has already started", ""));
			}
			match.setStatus("Live");
			match.setStart_time(LocalTime.now());
			matchRepository.save(match);
			TeamScore teamOneScore = new TeamScore(matchId, currentMatch.get().getTeam_1_id(), false, 0, 0, 0, 0, 0, 0,
					0);
			TeamScore teamTwoScore = new TeamScore(matchId, currentMatch.get().getTeam_2_id(), false, 0, 0, 0, 0, 0, 0,
					0);
			teamScoreRepository.save(teamOneScore);
			teamScoreRepository.save(teamTwoScore);
			List<TeamPlayerEntity> teamOnePlayers = teamPlayerRepository
					.findByTeamId(currentMatch.get().getTeam_1_id());
			List<TeamPlayerEntity> teamTwoPlayers = teamPlayerRepository
					.findByTeamId(currentMatch.get().getTeam_2_id());
